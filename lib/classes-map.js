...
var ClassesMap = (function () {
    function ClassesMap(java, options) {
        ...
        this.includedPatterns = Immutable.Set(_.map(this.options.packages, function (str) {
            if (/\.\*$/.test(str)) {
    ...
    ClassesMap.prototype.loadAllClasses = function (requiredClasses) {
        var work = new Work();
        _.forEach(requiredClasses, function (className) { return work.addTodo(className); });
        _.forEach(requiredCoreClasses, function (className) { return work.addTodo(className); });
        while (!work.isDone()) {
    ...
    ClassesMap.prototype.analyzeIncludedClasses = function () {
        ...
        var seeds = this.allClasses.toArray();
    ...
    ClassesMap.prototype.initialize = function () {
        var _this = this;
        return BluePromise.resolve().then(function () { return _this.preScanAllClasses(); }).then(function () { return _this.createShortNameMap(); }).then(function () { return _this.analyzeIncludedClasses(); });
    };
